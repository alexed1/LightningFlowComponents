...
export default class sendBetterEmailAddNewMembers extends LightningElement {
  ...
  showStringVariableModal() {
    if (
      this.selectedType === this.settings.stringVariablesOption &&
  ...
  searchEventHandler(event) {
    const searchString = event.detail.value.trim().replace(/\*/g).toLowerCase();
  ...
  actuallySearch(preventShowStringVariableModal) {
    if (
      this._customDataStructure &&
      this.selectedType &&
      this._customDataStructure[this.selectedType]
    ) {
      this.searchDisabled = true;
      let valueFieldName =
        this._customDataStructure[this.selectedType].valueFieldName;
      let labelFieldName =
        this._customDataStructure[this.selectedType].labelFieldName;

      this.searchResults = this._customDataStructure[this.selectedType].data
        .filter((curItem) => {
          return curItem[valueFieldName]
            .toLowerCase()
            .includes(this.searchString ? this.searchString.toLowerCase() : "");
        })
        .map((curItem) => {
          return {
            label: curItem[labelFieldName],
            value: curItem[valueFieldName],
            iconName: this.getSelectedIconName(
              this.existingMembers.includes(curItem[valueFieldName])
            )
          };
        });
  ...
  updateSelected() {
    this.searchResults = this.searchResults.map((curResult) => {
      return {
        ...curResult,
        iconName: this.getSelectedIconName(
          this.existingMembers.includes(curResult.value)
        )
      };
    });
  }
  ...
  handleRowAction(event) {
    ...
    if (rowValue && this.singleSelect) {
      ...
      if (rowValue && !this.existingMembers.includes(rowValue)) {
        ...
        this.existingMembers = this.existingMembers.filter(
          (curMember) => curMember !== rowValue
        );
      }
    }
  ...
  dispatchValueChangedEvent(value) {
    ...
    if (!returnedValue) {
      returnedValue = this.singleSelect
        ? this.existingMembers.length
          ? this.existingMembers[0]
          : null
        : this.existingMembers;
