...
export default class SendHTMLEmail extends LightningElement {
	...
	convertContextIntoRoleManagerParams(builderContext) {
		let allowedTypes = ['User', 'Lead', 'Contact', 'String'];
		let valueFieldName = 'name';
		let labelFieldName = 'label';
		let outputTypes = {};
		builderContext.recordLookups.forEach(curLookUp => {
		...
		let stringVariables = builderContext.variables.filter(curValue => {
			...
			if (!baseLabel || baseLabel === curRecipient.baseLabel) {
				Object.keys(curRecipient.typeMap).forEach(curType => {
					if (newValueObjectType !== curType) {
						valuesToCleanUp = valuesToCleanUp.concat(this._inputVariables.filter(curValue => curValue.id === curRecipient.typeMap[curType]));
					}
				});
			}
		});
	...
	handleFlowComboboxValueChange(event) {
		...
		if (this.inputValues[elementName]) {
			...
			this.dispatchComboboxFlowValueChangeEvent(elementName, formattedValue, event.detail.newValueDataType);
		}
	}
